...
boblib::utils::DataMap profileData;
boblib::utils::Profiler profiler;
boblib::camera::QhyCamera qhyCamera;
boblib::utils::TextWriter textWriter(cv::Scalar{190, 190, 190, 0}, 36, 2.0);
// boblib::utils::AutoExposure autoExposureControl(0.25, 180, 0.01, 100); // Tuned
boblib::utils::AutoExposure autoExposureControl(0.25, 200, 0.01, 100); // Experimental
boblib::utils::BrightnessEstimator brightnessEstimator;
boblib::utils::EntropyEstimator entropyEstimator;
boblib::utils::SubSampler subSampler(50, 50);
boblib::utils::AutoWhiteBalance autoWhiteBalance;
boblib::utils::BinImage bin_image;
boblib::utils::ImageStacker image_stacker;
boblib::blobs::ConnectedBlobDetection blob_detection(boblib::blobs::ConnectedBlobDetectionParams(7, 49, 40, 100));
...
int main(int argc, const char **argv)
{
    ...
    qhyCamera.set_control(boblib::camera::QhyCamera::ControlParam::Exposure, (argc > 1 ? atoi(argv[1]) : 10000));
    qhyCamera.set_control(boblib::camera::QhyCamera::ControlParam::Gain, 0.0);
    ...
    boblib::utils::WhiteBalanceValues previousGains;
    ...
    while (run)
    {
        ...
        if (!pauseCapture)
        {
            ...
            if (doEqualization)
            {
                ...
                frame = boblib::utils::Utils::equalize_image(frameDebayered, frameDebayered, clipLimit, doSoftwareBin ? eq_grid_size_mono : eq_grid_size_color);
            ...
            if(autoExposureControl.is_day())
            {
                if (doAutoWhiteBalance)
                {
                        ...
                        newGains.blue != previousGains.blue)
                    {
                        ...
                        qhyCamera.set_control(boblib::camera::QhyCamera::ControlParam::RedWB, newGains.red);
                        qhyCamera.set_control(boblib::camera::QhyCamera::ControlParam::GreenWB, newGains.green);
                        qhyCamera.set_control(boblib::camera::QhyCamera::ControlParam::BlueWB, newGains.blue);
            ...
            if (doAutoExposure)
            {
                ...
                qhyCamera.set_control(boblib::camera::QhyCamera::ControlParam::Exposure, exposure);
                qhyCamera.set_control(boblib::camera::QhyCamera::ControlParam::Gain, (int)gain);                    
            ...
            if (isBoxSelected)
            {
                ...
                noise_level = boblib::utils::Utils::estimate_noise(cropFrame);
                sharpness = boblib::utils::Utils::estimate_sharpness(cropFrame);
                noise_buffer.push_back(noise_level);
                sharpness_buffer.push_back(sharpness);
                profiler.stop("Metrics");

                auto sharpness_graph = boblib::utils::Utils::draw_graph("Sharpness", sharpness_buffer, cv::Size(200, 100), cropFrame.type(), cv::Scalar(0.0, 255.0, 255.0, 0.0), cv::Scalar(0.0, 25.0, 25.0, 0.0));
                auto noise_graph = boblib::utils::Utils::draw_graph("Noise", noise_buffer, cv::Size(200, 100), cropFrame.type(), cv::Scalar(0.0, 165.0, 255.0, 0.0), cv::Scalar(0.0, 5.0, 25.0, 0.0));
                boblib::utils::Utils::overlay_image(cropFrame, sharpness_graph, cv::Point(0, cropFrame.size().height - sharpness_graph.size().height), 0.7);
                boblib::utils::Utils::overlay_image(cropFrame, noise_graph, cv::Point(cropFrame.size().width - noise_graph.size().width, cropFrame.size().height - noise_graph.size().height), 0.7);
            ...
            textWriter.write_text(displayFrame, "Exp: " + boblib::utils::Utils::format_double((double)qhyCamera.get_camera_params().exposure / 1000.0, 2) + " ms Gain: " + std::to_string(qhyCamera.get_camera_params().gain), 1);
            ...
            textWriter.write_text(displayFrame, "Camera FPS: " + boblib::utils::Utils::format_double(profileData["GetImage"].fps(), 2), 1, true);
            textWriter.write_text(displayFrame, "Frame FPS: " + boblib::utils::Utils::format_double(profileData["Frame"].fps(), 2), 2, true);
            textWriter.write_text(displayFrame, "Entropy: " + boblib::utils::Utils::format_double(entropy, 2), 3, true);
            textWriter.write_text(displayFrame, get_running_time(starting_time), 36, true);
            if (qhyCamera.get_camera_info()->is_cool)
            {
                textWriter.write_text(displayFrame, boblib::utils::Utils::format_double(qhyCamera.get_current_temp()) + "c", 36);
            }
            ...
            features_enabled += qhyCamera.get_camera_params().cool_enabled ? "Cooling: " + boblib::utils::Utils::format_double(qhyCamera.get_camera_params().target_temp) +  " C " : "";
            ...
            // if (circleSet)
            // {
            //     auto roi_mask = boblib::utils::RoiMaskCalculator::calc_roi_mask(displayFrame, circleInit, circleEnd, 180.0, 220.0);
            ...
            if (showHistogram)
            {
                ...
                cv::Mat hist = boblib::utils::Utils::create_histogram(frameDebayered);
...
void createControlPanel()
{
    ...
    cv::createTrackbar("USB Traffic:", "", nullptr, maxUsbTraffic, changeTrackbars, (void *)(long)boblib::camera::QhyCamera::ControlParam::UsbTraffic);
    ...
    cv::createTrackbar("Gain:", "", nullptr, maxGain, changeTrackbars, (void *)(long)boblib::camera::QhyCamera::ControlParam::Gain);
    cv::setTrackbarPos("Gain:", "", (int)qhyCamera.get_camera_params().gain);
    int maxOffset = (int)qhyCamera.get_camera_info()->offset_limits.max;
    cv::createTrackbar("Offset:", "", nullptr, maxOffset, changeTrackbars, (void *)(long)boblib::camera::QhyCamera::ControlParam::Offset);
    ...
    cv::createTrackbar("Gamma:", "", nullptr, maxGamma, changeTrackbars, (void *)(long)boblib::camera::QhyCamera::ControlParam::Gamma);
    ...
    cv::createTrackbar("Red WB:", "", nullptr, maxRedWB, changeTrackbars, (void *)(long)boblib::camera::QhyCamera::ControlParam::RedWB);
    ...
    cv::createTrackbar("Green WB:", "", nullptr, maxGreenWB, changeTrackbars, (void *)(long)boblib::camera::QhyCamera::ControlParam::GreenWB);
    ...
    cv::createTrackbar("Blue WB:", "", nullptr, maxBlueWB, changeTrackbars, (void *)(long)boblib::camera::QhyCamera::ControlParam::BlueWB);
    ...
    cv::createTrackbar("Temperature Control:", "", nullptr, temperature_limits.max, changeTrackbars, (void *)(long)boblib::camera::QhyCamera::ControlParam::Cooler);
...
void treatKeyboardpress(int key)
{
    ...
    case '-':
        ...
        break;
    case '1':
        qhyCamera.set_control(boblib::camera::QhyCamera::ControlParam::TransferBits, 8);
        break;
    case '2':
        qhyCamera.set_control(boblib::camera::QhyCamera::ControlParam::TransferBits, 16);
        break;
    case 'g':
    ...
    case 'm':
        {
            ...
            auto new_bin = current_bin == boblib::camera::QhyCamera::Bin1x1 ? boblib::camera::QhyCamera::Bin2x2 : boblib::camera::QhyCamera::Bin1x1;
...
void changeTrackbars(int value, void *paramP)
{
    ...
    else if ((boblib::camera::QhyCamera::ControlParam)param == boblib::camera::QhyCamera::ControlParam::Gamma)
    {
        qhyCamera.set_control((boblib::camera::QhyCamera::ControlParam)param, (double)value / 10.0);
        return;
    }
    qhyCamera.set_control((boblib::camera::QhyCamera::ControlParam)param, (double)value);
}
...
void TransferbitsCallback(int, void*userData)
{
    ...
    qhyCamera.set_control(boblib::camera::QhyCamera::ControlParam::TransferBits, transferBits);
}
...
void drawOneFov(cv::Mat& frame, cv::Point2d center, double fov, double max_fov, const cv::Scalar& color)
{
    if (max_fov >= fov)
    {
        ...
        std::string fov_text = boblib::utils::Utils::format_double(fov, 2);
