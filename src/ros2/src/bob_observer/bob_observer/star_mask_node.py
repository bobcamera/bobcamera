...
class StarMaskNode(Node):
  ...
  def star_mask_maker(self):
    try:
        if os.path.exists("assets/masks/detection-mask.jpg"):
            shutil.copy("assets/masks/detection-mask.jpg", "assets/masks/detection-mask-copy.jpg")
            mask = cv2.imread("assets/masks/detection-mask-copy.jpg", cv2.IMREAD_GRAYSCALE)
        else:
            self.get_logger().info("detection-mask.jpg not found. Unable to create star mask.")
            return
        ...
        if self.msg_image != None:
            ...
            match self.day_night:
                case DayNightEnum.Day:
                    # self.get_logger().info(f'{self.get_name()} Star mask only applied at night')  
                    pass
                case DayNightEnum.Night:
                    ...
                    cv2.imwrite("assets/masks/detection-mask.jpg", combined_mask)
                    self.get_logger().info("Detection mask updated with star positions")                    
                case _:
