...
class TrackProvider final
{
public:
    COMPOSITION_PUBLIC
    explicit TrackProvider(ParameterNode &node,
...
private:
    ...
    {
        if ((resize_height_ <= 0) || node_.count_subscribers(std::string(pub_tracker_tracking_->get_topic_name()) + "/resized") <= 0)
        {
            return;
        }
